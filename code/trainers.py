...
class BasicTrainer(object):
    ...
    def tisdpo_concatenated_forward(self, model: nn.Module, reference_model: nn.Module,
        ...
        reference_all_logits, _ = self.DSKD.compute_dual_space_kd_loss_with_cma(self, concatenated_batch, teacher_concatenated_batch, distiller, model, reference_model)
        ...
        for batch in self.train_iterator:
            ...
            if config.loss.name in {'tisdpo'}:
                ...
                for microbatch_idx in range(self.config.gradient_accumulation_steps):
                    ...
                    t2s_logits, target = self.DSKD.compute_dual_space_kd_loss_with_cma(self, concat_student_data, concat_teacher_data, distiller, self.policy, self.reference_model)
