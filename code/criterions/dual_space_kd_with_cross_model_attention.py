...
class DualSpaceKDWithCMA(VariousDivergence):
    ...
    def compute_batch_dual_space_kd_loss_with_cma(
        ...
        stu_input_embeds = stu_embed_tokens(formal_input).clone().detach()   
        stu_target_embeds = stu_embed_tokens(formal_target).clone().detach().to(device)
        ...
        tea_input_embeds = tea_embed_tokens(formal_teacher_input).clone().detach().to(device)
        tea_target_embeds = tea_embed_tokens(formal_teacher_target).clone().detach().to(device)
